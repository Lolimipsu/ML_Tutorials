...
def calculate_demographic_data(print_data=True):
    ...
    filter1 = df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])
    filter2 = df['salary'] == '>50K'

    higher_education = round((filter1 & filter2).sum() / filter1.sum() * 100, 1)
    lower_education = round((~filter1 & filter2).sum() / (~filter1).sum() * 100, 1)
    ...
    rich_percentage = round((num_min_workers & filter2).sum() / num_min_workers.sum() * 100,1)
